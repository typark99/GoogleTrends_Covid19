...
directory2period = {
    "fox_news": (datetime.date(2020,2,1), datetime.date(2020,5,10)),
    "cnn": (datetime.date(2020,2,1), datetime.date(2020,5,10)),    
    "cdc": (datetime.date(2020,2,1), datetime.date(2020,5,10))        
}
...
for r_index, row in df.iterrows():
    ...
    for keyword_for_directory in directory2keywords:
        ...
        if (END_TIME-START_TIME).days < TIME_WINDOW:
            ...
            logger.info("TIME_WINDOW is adjusted [{}]".format(TIME_WINDOW))
